...
public class IncidentSearchService {
	...
	public static ArrayList<Incident> incidentSearch(IncidentSearch incidentSearch) {
		ArrayList<Incident> SearchResult = null;
		try {
			Log.d(TAG, "Service Start!!!!!!!!!!!!!!!!!!" );
			path = SettingService.getUrl() + "IncidentCLServlet?search=list";
			url = new URL(path);
			conn = (HttpURLConnection) url.openConnection();
			conn.setConnectTimeout(5000);
			conn.setRequestMethod("POST");
			Log.d(TAG, "Start encrypt" );
			String outputStr = DES.encryptDES(gson.toJson(incidentSearch), DES.KEY);
			Log.d(TAG, "Json : " + gson.toJson(incidentSearch));
